...
export default function Quiz1Practice({lecture,onLectureChange,onOpen}:{lecture:number;onLectureChange:(lecture:number)=>void;onOpen:(docId:string,page:number,quote?:string)=>void}){
 ...
 const bank=getSlidePracticePool(lecture),state=parseMarkedSet(raw,bank),review=state?.review;
 ...
 function start(){if(!available)return;save(key,startMarkedSet(bank,kind,count,state));resetFace();setNotice('');}
