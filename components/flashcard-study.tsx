...
export default function FlashcardStudy({week,topic,onOpen}:{week:number;topic:string;onOpen:(docId:string,page:number,quote?:string)=>void}){
  ...
  async function generate(){
    ...
    try{
      const response=await fetch('/api/flashcards',{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.any([controller.signal,AbortSignal.timeout(60000)]),body:JSON.stringify({week,docId:selected||undefined,count})});
      ...
      save(key,next);setRevealed(null);setWarning(data.warning||'');
