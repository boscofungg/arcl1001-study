...
export default function Home() {
 const [week,setWeek]=useState(1), [view,setView]=useState<'tutor'|'practice'|'flashcards'>('tutor');
 ...
 const [activeDocument,setActiveDocument]=useState<{id:string;page:number;highlight?:SourceHighlight}>({id:documents[0].id,page:documents[0].startPage});
 ...
 const filtered=documents.filter(doc=>(!lectureFilter||doc.week===lectureFilter)&&(doc.title+' '+doc.kind).toLowerCase().includes(query.toLowerCase()));
 ...
 function askAboutPage(doc:Doc,page:number){setPageContext({docId:doc.id,page});setScope('all');setDraft('Explain the evidence on this page briefly. Connect it with another Quiz 1 source if relevant.');setReader(null);setView('tutor');setMobileChat(true);input.current?.focus();}
 async function ask(question:string){
   ...
   try{
     const response=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json','Accept':'text/event-stream'},signal:AbortSignal.any([controller.signal,AbortSignal.timeout(65000)]),body:JSON.stringify({question:q,stream:true,week:scope==='all'?0:Number(scope),...(pageContext||{}),history})});
 ...
 return <div className="shell quiz1-shell">
  <aside className="sidebar quiz1-sidebar">
    ...
    <p className="q1-sidebar-note">Focused on the selected Quiz 1 materials. Practice questions are study aids, not the official paper.</p>
  </aside>
  <div className="workspace quiz1-workspace">
    <header className="topbar"><div><span>QUIZ 1</span><ChevronRight size={13}/><strong>{view==='tutor'?'Read & ask':view==='practice'?'Practice':'Flashcards'}</strong></div><div className="topbar-right"><ThemeToggle/>{view==='tutor'&&<button className="mobile-tutor-button" onClick={()=>setMobileChat(true)}><MessageSquare size={16}/> Tutor</button>}</div></header>
