...
export async function POST(request: Request) {
  ...
  function fallback(status = 422) {
    ...
    if (cards.length) return success(cards, `Using ${cards.length} prepared practice cards from your selected materials. These cards are curated, not AI-generated.`, true);
    return Response.json({ error: 'A reliable generated set is not available for this selection. Choose all materials for this lecture, or use the ready-made Quiz 1 practice sets.' }, { status });
  }
