...
export async function POST(request: Request) {
  ...
  if(typeof question!=='string' || !question.trim() || question.length>2000 || !Number.isInteger(week) || week<0 || week>3 || (docId && !documents.some(d=>d.id===docId)) || !Array.isArray(history) || history.length>8 || history.some((h: {role?:string;text?:string})=>!h || !['user','assistant'].includes(h.role||'') || typeof h.text!=='string' || h.text.length>4000)) return Response.json({error:'Please enter a question of up to 2,000 characters.'},{status:400});
  ...
  const found=retrieve(question+(isFollowup?' '+recentQuestion:''),week,selectedSource?undefined:docId);
  const sources=selectedSource?[selectedSource,...found.filter(source=>source.docId!==selectedSource.docId||source.page!==selectedSource.page)].slice(0,7):found;
