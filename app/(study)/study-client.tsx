...
export default function StudyWorkspace({initialContext,initialView='tutor'}:{initialContext?:PageContext;initialView?:'tutor'|'practice'}) {
 ...
 useEffect(()=>{
  ...
  try{Promise.resolve(context.registerTool({name:'open_course_week',description:'Open a supplied ARCL1001 week in the course browser.',inputSchema:{type:'object',properties:{week:{type:'integer',enum:weeks.map(unit=>unit.n)}},required:['week'],additionalProperties:false},annotations:{readOnlyHint:false},execute:async(value:unknown)=>{const n=(value as {week?:number})?.week;if(!weeks.some(w=>w.n===n))throw Error('Choose an available lecture');setWeek(n!);setLectureFilter(n!);setView('tutor');const doc=documents.find(d=>d.week===n&&d.kind==='Lecture')!;setActiveDocument({id:doc.id,page:doc.startPage});setQuery('');return{week:n,topic:weeks.find(w=>w.n===n)!.topic};}},{signal:lifecycle.signal})).catch(()=>{});}catch{}return()=>lifecycle.abort();
 },[]);
